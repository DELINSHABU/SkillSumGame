...
- Text-to-speech functionality (mentioned in README but not yet implemented)
- Math hints and jokes (mentioned in README but not yet implemented)
...
1. **Missing API Routes**: The following API endpoints are referenced but not implemented:
   ...
   These should be created as Next.js Route Handlers in `src/app/api/gemini-text/route.ts` and `src/app/api/gemini-tts/route.ts`
...
- React 19's enhanced features are available but not currently utilized
